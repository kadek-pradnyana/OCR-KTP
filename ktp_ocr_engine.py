...
def process_extract_entities(ocr_path):
        ...
        # print(text_response)
        # print('akan diextract')
        ktp_extract = extract_ktp_data(text_response)
        # print(ktp_extract.iloc[0].to_dict())
        # print(ktp_extract.iloc[0].split)

        # return (ktp_extract.iloc[0]).splitlines()
        return ktp_extract.iloc[0].to_dict() 
...
if __name__ == '__main__':
    if(len(sys.argv) > 1):
        # input: ocr file path
        ocr_path = sys.argv[1]
        # print('2. Extracting data from '+ocr_path)
